...
if (-not (Test-Path $javaExe)) {
    Write-Error "Java not found at: $javaExe (JAVA_HOME=$env:JAVA_HOME)"
    exit 1
}
...
$allArgs = @()
$allArgs += $defaultJvmOpts
if ($env:JAVA_OPTS) { $allArgs += $env:JAVA_OPTS }
if ($env:MAVEN_OPTS) { $allArgs += $env:MAVEN_OPTS }
$allArgs += "-cp", $classpath
$allArgs += "org.apache.maven.wrapper.MavenWrapperMain"
$allArgs += $MavenArgs
