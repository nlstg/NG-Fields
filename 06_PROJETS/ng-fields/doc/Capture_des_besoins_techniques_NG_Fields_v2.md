...
│  │  ├── NotificationService (FCM + APNs + Email + WhatsApp)                    ││
│  │  ├── SyncService (résolution conflits hors-ligne)                           ││
│  │  ├── OpenProjectService (API REST v3)                                       ││
...
││  Flyway (schema  │  │      sync/whatsapp) │  │  URLs signées (72 h)           ││
││  migrations)     │  └─────────────────────┘  └────────────────────────────────┘ │└──────────────────┘                                                             │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
║  │  │  ├── OpenProjectSvc      │    │          protocol/openid-connect/certs       ││  ║
...
║  │  │  └── Queues asynchrones :                                                    ││  ║
║  │  │       pdf_queue | email_queue | push_queue | sync_queue | whatsapp_queue    ││  ║
║  │  └─────────────────────────────────────────────────────────────────────────────┘│  ║
║  │                                                                                   │  ║
║  │  ┌─────────────────────────────────────────────────────────────────────────────┐ │  ║
...
╚══════════════════════════════════════════════════════════════════════════════════════════╝
      ...
      ▼                  ▼                  ▼                      ▼                ▼
┌──────────────┐  ┌───────────────┐  ┌───────────────┐  ┌──────────────────┐  ┌───────────┐
│ Supabase     │  │ OpenProject   │  │ FCM (Android) │  │ WhatsApp         │  │ SendGrid  │
│ PostgreSQL   │  │ API REST v3   │  │ APNs (iOS)    │  │ Business API     │  │ / SMTP    │
│ (Cloud)      │  │ (NG-STARs)    │  │               │  │ (Meta Cloud)     │  │           │
│ ├── SSL AES  │  │ API Token     │  │ Firebase      │  │ Bearer Token     │  │ API Key   │
│ ├── Backup   │  │ Bearer Auth   │  │ Admin SDK     │  │                  │  │ Port 587  │
│ └── Flyway   │  │               │  │               │  │                  │  │           │
│   migrations │  │               │  │               │  │                  │  │           │
└──────────────┘  └───────────────┘  └───────────────┘  └──────────────────┘  └───────────┘
```
